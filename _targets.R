...
# runs all of the network analyses
...
data_processing_targets <- tar_plan(
  ...
  authorship_data = clean_authorship_data(authorship_data_raw, 
                                          income_data_clean),
...
analysis_targets <- tar_plan(
  ...
  ## Country income by authorship position breakdown
  tar_target(income_position_summary,
             summarize_income_by_position(auths_with_gender)
             ),

  ## Gender over time by country income
...
outputs_targets <- tar_plan(
  ...
  ## Authorships by gender, position, country income (FIG 2)
...
plotting_targets <- tar_plan(
  ...
  tar_target(FigS2_tif,
             ggsave("figures/FigS2.tiff", gender_position_plot, 
                    height = 6, width = 6,  
                    units = "in", dpi = 600,
                    compression = "lzw")),
  ...
  tar_target(FigS3_tif,
             ggsave("figures/FigS3.tiff", country_income_plot, 
                    height = 9, width = 7,  
                    units = "in", dpi = 600,
                    compression = "lzw")),
  ...
  tar_target(FigS4_tif,
             ggsave("figures/FigS4.tiff", gender_country_plot, 
                    height = 6, width = 7,  
                    units = "in", dpi = 600,
                    compression = "lzw")),
  
  # Figure S5
  tar_target(FigS5_tif,
             ggsave("figures/FigS5.tiff", income_position_plot, 
                    height = 6, width = 8,  
                    units = "in", dpi = 600,
                    compression = "lzw")),
)
...
report_targets <- tar_plan(
  ...
  tar_render(
    results, path = "reports/results.Rmd",
