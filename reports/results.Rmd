...
tar_load(prolific_summary) # Table S2
```
...
Filtering the EHA research outputs catalog to peer-reviewed journal articles published from January 1, 2011 to December 31, 2022 resulted in a corpus of `r n_pub` articles. We identified `r n_auth` FLAs associated with those `r n_pub` articles, which were linked to `r n_contrib` unique authors.
...
# % of first and last authors within all FLAs
...
# last authorships as percent of FLAs for prolific authors
last_auth_65 <- nrow(prolific_summary %>% dplyr::filter(perc_last_auth > 65))
...
# gender and income within all FLAs
...
After combining the results of our two gender classification approaches (see **Text S2** and **Table S1** for a comparison of the approaches), prioritizing gender inferred using pronouns in cases of dataset disagreement, we ultimately classified `r gender_by_final[2,2]` authors (`r gender_by_final[2,3]`%) as gendered male, `r gender_by_final[1,2]` (`r gender_by_final[1,3]`%) as gendered female, `r gender_by_final[3,2]` (`r gender_by_final[3,3]`%) as gendered nonbinary, and `r gender_by_final[4,2]` (`r gender_by_final[4,3]`%) as unknown gender. Of all FLAs, `r gender_auths_table[2,2]` (`r gender_auths_table[2,3]`%) were by gendered male authors, `r gender_auths_table[1,2]` (`r gender_auths_table[1,3]`%) were by gendered female authors, `r gender_auths_table[3,2]` (`r gender_auths_table[3,3]`%) was by a gendered nonbinary author, and `r gender_auths_table[4,2]` (`r gender_auths_table[4,3]`%) were by authors of unknown gender. Our gender classification process was least successful for authorships listing an affiliation with China, with `r china_summary[3,4]` out of `r country_tots[4,2]` authorships classified as unknown gender. Given that only one author was classified as gendered nonbinary, we included that author in figures (except where noted) but did not attempt to draw any conclusions about authorship patterns in regard to nonbinary authors.

For both the first and last author positions, there were more authorships by gendered male authors than by gendered female authors (first-male: `r perc_all[2,4]`% of all FLAs, first-female: `r perc_all[1,4]`%, last-male: `r perc_all[6,4]`%, last-female: `r perc_all[5,4]`%; **Fig. S1**). Nearly three-quarters of all authors (`r one_auth_all`%) had one authorship each (`r one_auth_males` gendered male authors and `r one_auth_females` gendered female authors) (**Fig. 1**). There was only `r n_prolif_female` gendered female author with ≥ `r prolif_cutoff` total FLAs, while there were `r n_prolif_male` gendered male authors with `r prolif_cutoff` - `r max_auths` total FLAs each. Collectively, those `r n_prolif_male` gendered male authors accounted for `r auths_by_prolif_male[1,1]` (`r auths_by_prolif_male[1,2]`%) of all FLAs in the corpus (**Fig. 1**). Last authorships made up >65% of FLAs for `r last_auth_65` of the `r nrow(prolific_summary)` most productive authors (**Table S2**).

<br>

##### *Intersections of gender, country income, and authorship position*

A total of `r n_countries` countries (high-income: n = `r income_table[1,2]`, upper-middle-income: n = `r income_table[4,2]`, lower-middle-income: n = `r income_table[3,2]`, low-income: n = `r income_table[2,2]`) were represented in authorship affiliations (**Fig. S2**). Most authorships listed a primary affiliation with the `r country_tots[1,1]` (`r country_tots[1,3]`%), `r country_tots[2,1]` (`r country_tots[2,3]`%), `r country_tots[3,1]` (`r country_tots[3,3]`%), `r country_tots[4,1]` (`r country_tots[4,3]`%), or the `r country_tots[5,1]` (`r country_tots[5,3]`%) (**Fig. S2**), together comprising `r top5`% of all FLAs. Within each of these five countries, there were more authorships by gendered male authors than authors of any other gender (**Fig. S3**). Most FLAs had a high-income country affiliation (`r econ_auth[1,3]`%), while other income groups were less well represented (upper middle: `r econ_auth[4,3]`%, lower-middle: `r econ_auth[3,3]`%, low: `r econ_auth[2,3]`%; **Fig. S4**). 
...
For authors with an HIC affiliation, there was an interplay between gender and author position (**Fig. 2**). Specifically, there were more first authorships than last authorships (`r first_fem_H_perc`% versus `r last_fem_H_perc`% of all FLAs) by gendered female authors, while there were more last than first authorships (`r last_male_H_perc`% versus `r first_male_H_perc`% of all FLAs) by gendered male authors. The data on authorships with an upper-middle-income, lower-middle-income, or low-income country affiliation were too limited to determine if differences by gender and author position were meaningful (**Fig. 2**).
...
n_nonspecific_pubs <- nrow(nonspecific_pubs)

US_geo <- publication_data_with_geo %>% 
  dplyr::filter(grepl("US", cc_of_study, fixed = T))

n_US_geo <- nrow(US_geo)
...
align_2011_2016 <- get_geo_align_summary(geo_focused_pubs_2016, 
                                         time_period = "2011-2016")

geo_focused_pubs_2022 <- geo_focused_pubs %>% 
  dplyr::filter(year > 2016)

align_2017_2022 <- get_geo_align_summary(geo_focused_pubs_2022, 
...
# when the first author geography does not "match" article geography,
# what country is the first author affiliated with?
first_auth_not_local <- geo_focused_pubs %>% 
  dplyr::select(cc_of_study, first_auth_cc, first_geo_match) %>% 
  dplyr::filter(first_geo_match == "no") %>% 
  dplyr::mutate(first_auth_iso3 = countrycode::countrycode(
    first_auth_cc, "iso2c", "iso3c")) %>% 
  # group_by(income_majority) %>% 
  group_by(first_auth_iso3) %>% 
  dplyr::summarise(n = n()) %>% 
  left_join(income_data_clean[, c("iso3", "income_majority")], 
            by = c("first_auth_iso3" = "iso3")) %>% 
  arrange(desc(n))

last_auth_not_local <- geo_focused_pubs %>% 
  dplyr::select(cc_of_study, last_auth_cc, last_geo_match) %>% 
  dplyr::filter(last_geo_match == "no") %>% 
  dplyr::mutate(last_auth_iso3 = countrycode::countrycode(
    last_auth_cc, "iso2c", "iso3c")) %>% 
  # group_by(income_majority) %>% 
  group_by(last_auth_iso3) %>% 
  dplyr::summarise(n = n()) %>% 
  left_join(income_data_clean[, c("iso3", "income_majority")], 
            by = c("last_auth_iso3" = "iso3")) %>% 
  arrange(desc(n))
...
We found that `r n_geo_pubs` articles (`r geo_perc`% of all articles in the corpus) were geographically focused on one or more non-US countries (e.g. field or labwork was performed there, specialized knowledge of a country was required to perform the research). About two-thirds of the time (`r either_match_perc`%), either the first or last authorship geography matched the article geography. First authorship geography matched article geography `r first_match_perc`% of the time, whereas last authorship geography matched article geography `r last_match_perc`% of the time. Both first and last authorships matched the article geography `r both_match_perc`% of the time. Though there were more articles published in the second half of the dataset (2011-2016: `r nrow(geo_focused_pubs_2016)` articles; 2017-2022: `r nrow(geo_focused_pubs_2022)` articles), the rates of authorship-geography matches stayed nearly constant over time (**Table S4**). There were `r n_nonspecific_pubs` articles (`r round((n_nonspecific_pubs/n_pub)*100, 1)`%) with a non-specific article geography and `r n_US_geo` articles (`r round((n_US_geo/n_pub)*100, 1)`%) with an article geography that included the United States.

<br>

##### *Characteristics of the author network*

See `_network_analysis_targets.R` file and **Table S5** for results reported in this section.

<br>


#### Figure 1
Fig 1. Total first and last authorships associated with unique authors. Gendered female (n = 186) and gendered male (n = 297) authors are displayed in the top and bottom panels respectively. Authors classified as gendered nonbinary (n = 1) or unknown gender (n = 14) are not displayed. Note that the y-axis is on a square-root scale.
